...
try {
  ...
  // Prevent deletion if tied to scheduled / checked-in appointments
  ...
  if (!empty($row['cnt']) && (int)$row['cnt'] > 0) {
    ...
    echo json_encode(['ok' => false, 'error' => 'Room has scheduled or checked-in appointments and cannot be deleted.']);
    exit;
  }

  $del = $mysqli->prepare("DELETE FROM Rooms WHERE id = ? LIMIT 1");
  $del->bind_param('i', $id);
  $del->execute();
  $del->close();
